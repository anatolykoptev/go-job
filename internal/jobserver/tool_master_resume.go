...
func registerMasterResumeBuild(server *mcp.Server) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Build a master resume from your full resume text. Parses into a structured knowledge graph (skills, experiences, projects, achievements) with vector embeddings for semantic search. DESTRUCTIVE: rebuilding when a profile already exists destroys the existing profile (skills, projects, experiences, achievements, educations, certifications, domains, methodologies, and upwork_profile data) and rebuilds from scratch. By default (replace=false) the call REFUSES if a profile exists and reports what would be destroyed; pass replace=true only when you intentionally want to replace the whole profile. A failed, timed-out, or cancelled rebuild leaves the existing profile intact (the clear and all inserts share one transaction). Run once for a fresh profile, then use resume_generate to create tailored versions.",
		...
		result, err := jobs.BuildMasterResume(ctx, input.Resume, input.Replace)
		if err != nil {
			return nil, nil, err
		}
