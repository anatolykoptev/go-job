...
spawn:
	...
	if llmUnavailable {
		if len(finalJobs) > 0 {
			...
			summary := fmt.Sprintf("Prose summary unavailable (LLM %s); %d listing(s) served from machine-extracted structured sources, ranked by relevance. The listings themselves are complete.", llmErrClass, len(finalJobs))
